...
function generateGrid() {
	...
	table = document.createElement("table");	// HTMLTableElement init
	table.id = "my_table";

	for (let i = 0; i < grid_size_y; i++) {

		let row = document.createElement("tr");		// bei problemen sonst einfärbig + border

		for (let j = 0; j < grid_size_x; j++) {

			let cell = document.createElement("td");	// schachbrettmuster style
			let class_name = "";
	...
	place_to_cell(start_pos[0], start_pos[1]).classList.add("startpunkt");
	place_to_cell(target_pos[0], target_pos[1]).classList.add("zielpunkt");
}
...
function add_wall(x, y) {
	...
	if (!cell.classList.contains("startpunkt") && !cell.classList.contains("zielpunkt")) {
		grid[x][y] = -1;
...
function click_event(event) {
	...
	if (clicking && event.target.classList.contains("cell")) {
		...
		if (moving_start && !event.target.classList.contains("zielpunkt")) {

			start_pos = place;
			document.querySelector(".startpunkt").classList.remove("startpunkt");
			event.target.classList.add("startpunkt");

			if (grid[place[0]][place[1]] < 0) {
				document.querySelector("#generator").value = "1";
		...
		else if (moving_target && !event.target.classList.contains("startpunkt")) {

			target_pos = place;
			document.querySelector(".zielpunkt").classList.remove("zielpunkt");
			event.target.classList.add("zielpunkt");
...
function visualEvents() {
	...
	document.querySelector("#my_table").addEventListener('mousedown', event => {
		...
		if (event.target.classList.contains("startpunkt"))
			moving_start = true;

		if (event.target.classList.contains("zielpunkt"))
			moving_target = true;
